          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {item.map((tt, tIndex) => (
              <React.Fragment key={tIndex}>
                <tr className={`bg-white hover:bg-gray-100 transition cursor-pointer ${tt['otb'] ? 'line-through text-red-500' : ''}`} onClick={(event) => handleGetChart(period_id, tt['points'], event)}>
